...
void genAssertType(scope ref ArrayBuilder!JsStatement out_, ref TranslateModuleCtx ctx, in Source source, in Type type, JsExpr get) {
	type.matchIn!void(
		(in Type.Bogus) {},
		(in TypeParamIndex _) {},
		(in StructInst x) {
			Opt!JsExpr notOk = x.decl.body_.isA!BuiltinType
				...
				: optIf(!x.decl.body_.isA!(StructBody.Extern), () =>
					genNot(ctx.alloc, source, genInstanceof(ctx.alloc, source, get, translateStructReference(ctx, source, x.decl))));
			if (has(notOk))
			...
			translateToBlockStatement(ctx.alloc, (scope ExprPos pos) =>
				translateInlineCall(
					exprCtx, source, impl.returnType, pos, decl, impl.paramTypes, impl.arity.as!uint, (size_t i) =>
						i == 0
							? genThis(source)
							: genLocalGet(source, localName(*decl.params.as!(Destructure[])[i].as!(Local*))))));
	} else {
			...
			map(ctx.alloc, x.parts, (ref Destructure part) =>
				immutable KeyValuePair!(JsMemberName, JsDestructure)(
					...
					JsDestructure(allocate(ctx.alloc, JsDefaultDestructure(
						translateDestructure(ctx, part),
						genThrowBogusExpr(ctx.alloc, noSource))))))));
}
			...
			foreach (size_t index, ref RecordField field; fields) {
				...
				if (index == fields.length - 1)
					...
					add(ctx.alloc, out_, genIf(
						ctx.alloc,
						source,
						genNotEqEq(ctx.alloc, source, genLocalGet(source, name), equal),
						genReturn(ctx.alloc, source, genLocalGet(source, name))));
				}
			}
	...
	*/
	matchUnionMembers(ctx, source, members, p0, (size_t memberIndex, ref UnionMember member) {
		...
		return genReturn(
			ctx.alloc,
			source,
			genTernary(ctx.alloc, source, genIn(ctx.alloc, source, JsMemberName.unionMember(member.name), p1), then, else_));
	});
JsExpr combineWithOr(T)(ref Alloc alloc, in Source source, in T[] xs, in JsExpr delegate(ref T) @safe @nogc pure nothrow cb) =>
...
JsExprOrBlockStatement translateEqualUnion(
	...
	matchUnionMembers(ctx, source, members, p0, (size_t memberIndex, ref UnionMember member) =>
		genReturn(ctx.alloc, source, genAnd(
			...
			genCallCompareProperty(ctx, source, auto_.members[memberIndex], p0, p1, JsMemberName.unionMember(member.name)))));
JsExpr genCallCompareProperty(ref TranslateExprCtx ctx, in Source source, Called called, JsExpr p0, JsExpr p1, JsMemberName name) =>
...
JsExpr genNewJson(ref TranslateModuleCtx ctx, in Source source, JsExpr[] pairs) =>
	genCallAwait(
		ctx.alloc,
		source,
		allocate(ctx.alloc, translateFunReference(ctx, source, ctx.program.commonFuns.newJsonFromPairs.decl)),
		pairs);
JsExpr genTuple(ref TranslateExprCtx ctx, in Source source, size_t nArgs, in JsExpr delegate(size_t) @safe @nogc pure nothrow cbArg) {
	switch (nArgs) {
		case 0:
	...
	if (fun.body_.isA!(FunBody.FileImport))
		return fun.body_.as!(FunBody.FileImport).content.matchIn!JsExprOrBlockStatement(
			(in immutable ubyte[] bytes) =>
				JsExprOrBlockStatement(allocate(ctx.alloc, genArray(source, map(ctx.alloc, bytes, (ref immutable ubyte x) =>
					genIntegerUnsigned(source, x))))),
			(in string s) =>
				JsExprOrBlockStatement(allocate(ctx.alloc, genString(source, s))),
			...
			return translateToExprOrBlockStatement(ctx.alloc, (scope ExprPos pos) =>
				withMapToStackArray!(ExprResult, Type, Destructure)(
					...
					(scope Type[] paramTypes) =>
						translateInlineCall(ctx, source, fun.returnType, pos, fun, paramTypes, params.length, (size_t i) =>
							translateLocalGet(source, params[i].as!(Local*)))));
		}
	}
}
...
Source source(in TranslateExprCtx ctx, in Expr expr) =>
	source(ctx, expr.range);
Source source(in TranslateExprCtx ctx, in Range range) =>
	source(ctx.ctx, UriAndRange(ctx.curUri, range), ctx.curFun.name);
...
JsStatement translateToStatement(ref Alloc alloc, in StatementsCb cb) {
	...
	return statements.length == 1 ? only(statements) : JsStatement(noSource, JsStatementKind(JsBlockStatement(statements))); // TODO : needs a source?
}
...
JsBlockStatement translateExprToSwitchBlockStatement(ref TranslateExprCtx ctx, ref Expr a, Type type) =>
	isVoid(type)
		? translateToBlockStatement(ctx.alloc, (scope ExprPos pos) =>
			forceStatements(ctx, source(ctx, a), pos, (scope ref ArrayBuilder!JsStatement out_, scope ExprPos inner) {
				ExprResult result = translateExpr(ctx, a, type, inner);
				assert(result.isA!(ExprResult.Done));
				add(ctx.alloc, out_, genBreakNoLabel(source(ctx, a)));
				return result;
			}))
...
ExprResult forceStatements(ref TranslateExprCtx ctx, in Source source, scope ExprPos pos, in StatementsCb cb) =>
	// TODO: 'source' only matters for iife, omit somehow? I think cb always adds at least one statement, so could use the first statement's source
	pos.match!ExprResult(
		(ExprPos.Expression) =>
...
ExprResult translateExpr(ref TranslateExprCtx ctx, ref Expr a, Type type, scope ExprPos pos) {
	...
	return a.kind.match!ExprResult(
		...
		(ClosureGetExpr x) =>
			forceExpr(ctx, pos, type, genLocalGet(source, localName(*x.local))),
		(ClosureSetExpr x) =>
		...
		(ref LoopBreakExpr x) {
			...
			assert(res.isA!(ExprResult.Done));
			if (isVoid(type))
				add(ctx.alloc, pos.as!(ExprPos.Statements*).statements, genBreak(source, JsName.noPrefix(symbol!"loop")));
			return res;
		},
		(LoopContinueExpr x) {
			assert(pos.isA!(ExprPos.Statements*));
		...
		(ref MatchUnionExpr x) =>
			translateMatchUnion(ctx, source, x, type, pos),
		(ref MatchVariantExpr x) =>
			translateMatchVariant(ctx, source, x, type, pos),
...
ExprResult translateCallOption(ref TranslateExprCtx ctx, in Source source, ref CallOptionExpr a, Type type, scope ExprPos pos) =>
	/*
	firstArg?.called
	==>
	...
	*/
	withTemp(ctx, symbol!"option", a.firstArg, pos, (JsName option, scope ExprPos inner) {
		JsExpr forceIt = genOptionForce(ctx.alloc, source, genLocalGet(source, option));
		...
		return forceExpr(ctx, inner, type, genTernary(
			ctx.alloc,
			source,
			genOptionHas(ctx.alloc, source, genLocalGet(source, option)),
			then,
...
ExprResult translateInlineCall(
	...
	return called.body_.matchIn!ExprResult(
		...
		(in FunBody.CreateUnion x) {
			JsExpr member = genPropertyAccess(ctx.alloc, source, returnTypeRef, JsMemberName.unionConstructor(x.member.name));
		...
		(in FunBody.UnionMemberGet x) =>
			withTemp2(ctx, symbol!"member", onlyArg(), pos, (JsName member, scope ExprPos inner) {
				...
				return forceExpr(ctx.alloc, inner, returnType, genTernary(
					ctx.alloc,
					source,
					genIn(ctx.alloc, source, memberName, genLocalGet(source, member)),
					genOptionSome(ctx, source, returnType, genPropertyAccess(ctx.alloc, source, genLocalGet(source, member), memberName)),
		...
		(in FunBody.VariantMethod x) =>
			expr(genCall(
				...
				allocate(ctx.alloc, genPropertyAccess(ctx.alloc, source, getArg(0), JsMemberName.variantMethod(x.method.name))),
				args(skip: 1))),
		(in FunBody.VarSet x) =>
			forceStatement(ctx, pos, genAssign(ctx.alloc, source, translateVarReference(ctx.ctx, source, x.var), onlyArg())));
}
...
JsExpr translateBuiltinUnary(ref Alloc alloc, in Source source, BuiltinUnary a, JsExpr arg) {
	...
	final switch (a) {
		...
		case BuiltinUnary.arraySize:
			return genCallSync(alloc, source, BigInt, [genPropertyAccess(alloc, source, arg, JsMemberName.noPrefix(symbol!"length"))]);
		...
		case BuiltinUnary.countOnesNat64:
			...
			JsExpr digits = genCallPropertySync(alloc, source, Array, JsMemberName.noPrefix(symbol!"from"), [
				genCallPropertySync(alloc, source, arg, JsMemberName.noPrefix(symbol!"toString"), [genNumber(source, 2)])]);
			JsName x = JsName.specialLocal(symbol!"x");
			// x => x === "1"
			JsExpr fn = genArrowFunction(
				alloc, source, SyncOrAsync.sync, [JsDestructure(x)], genEqEqEq(alloc, source, genLocalGet(source, x), genString(source, "1")));
		...
		case BuiltinBinaryLazy.optionQuestion2:
			...
			return withTemp2(ctx, symbol!"option", left, pos, (JsName option, scope ExprPos inner) =>
				forceExpr(ctx.alloc, inner, type, genTernary(
					...
					source, 
					genOptionHas(ctx.alloc, source, genLocalGet(source, option)),
					genOptionForce(ctx.alloc, source, genLocalGet(source, option)),
					right)));
	}
}

JsExpr translateBuiltinBinaryMath(ref TranslateExprCtx ctx, in Source source, BuiltinBinaryMath kind, JsExpr left, JsExpr right) {
	...
	withTemp(ctx, symbol!"option", unpack.option, pos, (JsName option, scope ExprPos inner) =>
		forceStatement(ctx, inner, genIf(
			...
			translateToStatement(ctx.alloc, (scope ExprPos inner2) =>
				translateLetLikeCb(
					ctx, source, unpack.destructure, genOptionForce(ctx.alloc, source, genLocalGet(source, option)), inner2,
...
ExprResult translateLambda(ref TranslateExprCtx ctx, in Source source, ref LambdaExpr a, Type type, scope ExprPos pos) =>
...
ExprResult translateLet(ref TranslateExprCtx ctx, in Source source, ref LetExpr a, Type type, scope ExprPos pos) =>
	translateLetLike(ctx, source, a.destructure, translateExprToExpr(ctx, a.value, a.destructure.type), a.then, type, pos);
ExprResult translateLetLike(
	...
	translateLetLikeCb(ctx, source, destructure, value, pos, (scope ref ArrayBuilder!JsStatement, scope ExprPos inner) =>
...
JsExpr translateLocalGet(in Source source, in Local* local) =>
	genLocalGet(source, localName(*local));
					...
					*/
					...
					JsBlockStatement body_ = translateToBlockStatement(
						...
						(scope ref ArrayBuilder!JsStatement out_, scope ExprPos bodyPos) {
							add(ctx.alloc, out_, genAssign(ctx.alloc, source, option, translateExprToExpr(ctx, unpack.option)));
							add(ctx.alloc, out_, genIf(
								ctx.alloc,
								source,
								genOptionHas(ctx.alloc, source, genLocalGet(source, option)),
					...
					return translateLetLike(
						ctx, source, unpack.destructure, genOptionForce(ctx.alloc, source, genLocalGet(source, option)), a.after, type, outerPos);
				} else {
					/*
					while (true) {
						const option = <<option>>
...
ExprResult translateMatchEnum(ref TranslateExprCtx ctx, in Source source, ref MatchEnumExpr a, Type type, scope ExprPos pos) =>
	forceStatement(ctx, pos, genSwitch(
		...
		map(ctx.alloc, a.cases, (ref MatchEnumExpr.Case case_) =>
			JsSwitchStatement.Case(
				translateEnumValue(ctx.ctx, source, case_.member),
				translateExprToSwitchBlockStatement(ctx, case_.then, type))),
		translateSwitchDefault(ctx, source, a.else_, type, "Invalid enum value")));

ExprResult translateMatchIntegral(ref TranslateExprCtx ctx, in Source source, ref MatchIntegralExpr a, Type type, scope ExprPos pos) =>
	forceStatement(ctx, pos, genSwitch(
		ctx.alloc,
		source,
		translateExprToExpr(ctx, a.matched),
		map(ctx.alloc, a.cases, (ref MatchIntegralExpr.Case case_) =>
			JsSwitchStatement.Case(
				a.kind.isSigned ? genIntegerSigned(source, case_.value.asSigned) : genIntegerUnsigned(source, case_.value.asUnsigned),
				translateExprToSwitchBlockStatement(ctx, case_.then, type))),
...
ExprResult translateMatchStringLike(
	...
	forceStatement(ctx, pos, genSwitch(
		...
		map(ctx.alloc, a.cases, (ref MatchStringLikeExpr.Case case_) =>
			JsSwitchStatement.Case(genString(source, case_.value), translateExprToSwitchBlockStatement(ctx, case_.then, type))),
		translateExprToSwitchBlockStatement(ctx, a.else_, type)));

ExprResult translateMatchUnion(ref TranslateExprCtx ctx, in Source source, ref MatchUnionExpr a, Type type, scope ExprPos pos) =>
	withTemp(ctx, symbol!"matched", a.matched, pos, (JsName matched, scope ExprPos inner) =>
		translateMatchUnionOrVariant!(MatchUnionExpr.Case)(
			ctx, source, matched, a.cases, type, inner,
			translateSwitchDefault(ctx, source, has(a.else_) ? some(*force(a.else_)) : none!Expr, type, "Invalid union value"),
			(ref MatchUnionExpr.Case case_) =>
				MatchUnionOrVariantCase(
					genIn(ctx.alloc, source, JsMemberName.unionMember(case_.member.name), genLocalGet(source, matched)),
					genPropertyAccess(ctx.alloc, source, genLocalGet(source, matched), JsMemberName.unionMember(case_.member.name)))));

ExprResult translateMatchVariant(ref TranslateExprCtx ctx, in Source source, ref MatchVariantExpr a, Type type, scope ExprPos pos) =>
	withTemp(ctx, symbol!"matched", a.matched, pos, (JsName matched, scope ExprPos inner) =>
		translateMatchVariant(ctx, source, matched, a.cases, translateExprToBlockStatement(ctx, a.else_, type), type, inner));
ExprResult translateMatchVariant(
	...
	translateMatchUnionOrVariant!(MatchVariantExpr.Case)(
		ctx, source, matched, cases, type, pos, else_,
		(ref MatchVariantExpr.Case case_) =>
			MatchUnionOrVariantCase(
				genInstanceof(ctx.alloc, source, genLocalGet(source, matched), translateStructReference(ctx, source, case_.member.decl)),
				genLocalGet(source, matched)));
...
ExprResult translateMatchUnionOrVariant(Case)(
	...
	forceStatement(
		ctx, pos,
		foldReverse!(JsStatement, Case)(JsStatement(source, JsStatementKind(default_)), cases, (JsStatement else_, ref Case case_) {
...
JsBlockStatement translateSwitchDefault(ref TranslateExprCtx ctx, in Source source, Opt!Expr else_, Type type, string error) =>
	has(else_)
...
JsExpr translateEnumValue(ref TranslateModuleCtx ctx, in Source source, EnumOrFlagsMember* a) =>
	genPropertyAccess(ctx.alloc, source, translateStructReference(ctx, source, a.containingEnum), JsMemberName.enumMember(a.name));

ExprResult translateFinally(ref TranslateExprCtx ctx, in Source source, ref FinallyExpr a, Type type, scope ExprPos pos) =>
	/*
	finally right
	below
	==>
...
ExprResult translateTry(ref TranslateExprCtx ctx, in Source source, ref TryExpr a, Type type, scope ExprPos pos) {
	...
	return forceStatement(ctx, pos, genTryCatch(
		...
		translateToBlockStatement(ctx.alloc, (scope ExprPos inner) =>
			translateMatchVariant(
				ctx, source, exceptionName, a.catches,
				genBlockStatement(ctx.alloc, [genThrow(ctx.alloc, source, genLocalGet(source, exceptionName))]),
				type, inner))));
}

ExprResult translateTryLet(ref TranslateExprCtx ctx, in Source source, ref TryLetExpr a, Type type, scope ExprPos pos) =>
	/*
		...
		JsBlockStatement catchBlock = translateToBlockStatement(
			...
			(scope ref ArrayBuilder!JsStatement catchOut, scope ExprPos catchPos) {
				JsExpr cond = genOr(
					ctx.alloc,
					source,
					genNot(ctx.alloc, source, genLocalGet(source, catching)),
					genNot(
						ctx.alloc,
						source,
						genInstanceof(ctx.alloc, source, genLocalGet(source, exceptionName),
						translateStructReference(ctx, source, a.catch_.member.decl))));
				add(ctx.alloc, catchOut, genIf(ctx.alloc, source, cond, genThrow(ctx.alloc, source, genLocalGet(source, exceptionName))));
				return translateLetLike(
					ctx, source, a.catch_.destructure, genLocalGet(source, exceptionName), a.catch_.then, type, catchPos);
			});
...
JsExpr[] withSpecImpls(ref TranslateModuleCtx ctx, in Source source, in FunOrTest caller, in Called called, in JsExpr [] args) =>
...
JsExpr translateFunOrSpecReference(ref TranslateModuleCtx ctx, in Source source, in FunOrTest caller, in Called called) =>
	called.match!JsExpr(
		(ref Called.Bogus x) =>
		...
		(CalledSpecSig x) =>
			genLocalGet(
				source,
				JsName(JsName.Kind.specSig, x.nonInstantiatedSig.name, some(safeToUshort(findSigIndex(*caller.as!(FunDecl*), x))))));
	...
	if (isEmpty(specImpls))
		...
		return genArrowFunction(
			...
			JsExprOrBlockStatement(allocate(ctx.alloc, genCallWithSpread(
				ctx.alloc, source, SyncOrAsync.sync, f, specImpls, genLocalGet(source, args)))));
	}
}
