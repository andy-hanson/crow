...
char constantarr_0_578[1];
char constantarr_0_579[1];
...
char constantarr_0_578[1] = "x";
char constantarr_0_579[1] = "y";
...
struct arr_0 get_fun_name_98(uint64_t fun_id) {switch (fun_id) {
		...
		case 397: {
			return (struct arr_0) {13, constantarr_0_583};
		}
		default:
...
struct fut_0* main_0(struct ctx* ctx, struct arr_1 _p0) {
	...
	assert(ctx, (m0.x == 1u));
	assert(ctx, (m0.y == 2u));
	struct arr_0 _0 = to_str_3(ctx, m0.x);
	print(_0);
	struct arr_0 _1 = to_str_3(ctx, m0.y);
	print(_1);
